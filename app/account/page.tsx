...
export default async function AccountOverview() {
  ...
  const stats = [
    { label: <T en="Total Orders" bn="মোট অর্ডার" />, value: String(total), c: "#2F90CC", icon: "M6 2h9l5 5v15H6zM14 2v6h6M9 13h6M9 17h6" },
    { label: <T en="Active" bn="চলমান" />, value: String(active), c: "#f59e0b", icon: "M1 3h15v13H1zM16 8h4l3 3v5h-7M5.5 18.5a2.5 2.5 0 105 0M18.5 18.5a2.5 2.5 0 105 0" },
    { label: <T en="Delivered" bn="ডেলিভারড" />, value: String(delivered), c: "#16a34a", icon: "M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10zM9 12l2 2 4-4" },
    { label: <T en="Total Spent" bn="মোট কেনাকাটা" />, value: taka(spent), c: "#DE6699", icon: "M3 7h18v10H3zM3 11h18M7 15h3" },
  ];

  return (
    <DashboardShell active="overview" name={name} email={session.email || ""}>
      <div className="mb-4">
        <h1 className="font-display text-2xl font-bold text-gray-900"><T en="Welcome" bn="স্বাগতম" />{name ? `, ${name.split(" ")[0]}` : ""} 👋</h1>
        <p className="mt-1 text-sm text-gray-500"><T en="A summary of your orders and account." bn="আপনার অর্ডার ও অ্যাকাউন্টের সারসংক্ষেপ।" /></p>
      </div>

      {/* Stat cards */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5">
        {stats.map((s) => (
          <div key={s.c} className="rounded-xl bg-white border border-black/[0.06] shadow-[0_1px_2px_rgba(0,0,0,0.04)] p-3">
      ...
      <div className="mt-5 rounded-2xl bg-white border border-black/[0.06] shadow-[0_1px_2px_rgba(0,0,0,0.04)] overflow-hidden">
        <div className="flex items-center justify-between px-4 sm:px-5 py-4 border-b border-black/5">
          <h2 className="font-semibold text-gray-900"><T en="Recent Orders" bn="সাম্প্রতিক অর্ডার" /></h2>
          <a href="/account/orders" className="text-sm font-semibold text-brand-dark hover:underline"><T en="See all" bn="সব দেখুন" /></a>
        </div>
        {recent.length === 0 ? (
          <div className="px-5 py-12 text-center">
            <p className="text-gray-500"><T en="No orders yet." bn="এখনো কোনো অর্ডার নেই।" /></p>
            <a href="/products" className="mt-3 inline-block rounded-lg bg-brand text-white px-5 py-2.5 text-sm font-semibold hover:bg-brand-dark transition-colors"><T en="Start shopping" bn="শপিং শুরু করুন" /></a>
          </div>
        ) : (
