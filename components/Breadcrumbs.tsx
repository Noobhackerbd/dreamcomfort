          ...
          {items.map((it, i) => {
            ...
            return (
              <li key={it.href} className="flex items-center gap-1.5 min-w-0">
                {last ? (
                  <span className="text-gray-700 font-medium truncate max-w-[220px]" aria-current="page">{it.name}</span>
                ) : (
                  <a href={it.href} className="hover:text-brand truncate">{it.name}</a>
                )}
