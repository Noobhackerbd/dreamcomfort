    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-[95] bg-black/50 backdrop-blur-sm" onClick={() => setOpen(false)}>
          <div className="mx-auto max-w-2xl px-0 sm:px-4 sm:mt-[7vh]">
            <div className="bg-white rounded-b-2xl sm:rounded-2xl shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
              {/* Search bar */}
              <div className="flex items-center gap-2 p-3 sm:p-4">
                <div className="flex-1 flex items-center gap-2 h-11 rounded-full bg-gray-100 px-4 focus-within:ring-2 focus-within:ring-brand/25 transition">
                  ...
                  <input ref={inputRef} value={q} onChange={(e) => onChange(e.target.value)}
                    ...
                    placeholder={L("Search products...","পণ্য খুঁজুন...")} className="flex-1 min-w-0 bg-transparent text-[15px] outline-none" />
                  {q && <button onClick={() => { setQ(""); setItems([]); inputRef.current?.focus(); }} aria-label={L("Clear","মুছুন")} className="shrink-0 text-gray-400 hover:text-gray-600">
                  ...
                  <>
                    ...
                    {items.map((s) => (
                      <div key={s} className="flex items-center hover:bg-gray-50 transition">
                        ...
                        <button onClick={() => { setQ(s); onChange(s); inputRef.current?.focus(); }} aria-label={L("Fill in","ইনপুটে বসান")} className="shrink-0 h-9 w-9 grid place-items-center text-gray-300 hover:text-gray-500 mr-2">
