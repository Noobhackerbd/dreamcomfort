...
export function LoginModal() {
  ...
  async function sendOtp(e: React.FormEvent) {
    ...
    } catch (e: any) { setErr(e?.message ?? L("Failed.","ব্যর্থ।")); }
    finally { setBusy(false); }
  }
  ...
  async function submitName(e: React.FormEvent) {
    e.preventDefault();
    setErr(null);
    if (custName.trim().length < 2) { setErr(L("Please enter your name.","আপনার নাম লিখুন।")); return; }
    setBusy(true);
    try {
  ...
  async function oauthGoogle() {
    ...
    } catch (e: any) { setErr(e?.message ?? L("Failed.","ব্যর্থ।")); setBusy(false); }
  }
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/50 backdrop-blur-sm grid place-items-center px-4 py-6 overflow-y-auto" onClick={() => setOpen(false)}>
      <div className="w-full max-w-md rounded-2xl bg-white shadow-2xl p-6 sm:p-7 relative my-auto" onClick={(e) => e.stopPropagation()}>
        <button onClick={() => setOpen(false)} aria-label={L("Close","বন্ধ")} className="absolute right-4 top-4 h-8 w-8 grid place-items-center rounded-full text-gray-400 hover:bg-black/5 transition">
        ...
        <div className="text-center mb-5 mt-1">
          ...
          <h2 className="text-xl font-bold font-display text-gray-900">{L("Welcome","স্বাগতম")}</h2>
          <p className="text-[13px] text-gray-500 mt-1">{askName ? L("One more step","আর একটি ধাপ বাকি") : L("Log in to continue shopping","লগইন করে কেনাকাটা চালিয়ে যান")}</p>
        </div>

        {askName ? (
          <form onSubmit={submitName} className="space-y-3">
            <p className="text-center text-[13px] text-gray-500 -mt-2 mb-1">{L("Enter your name — it will be saved to your profile.","আপনার নামটি লিখুন — এটি আপনার প্রোফাইলে সংরক্ষিত থাকবে।")}</p>
            <div className="relative">
              ...
              <input value={custName} onChange={(e) => setCustName(e.target.value)} placeholder={L("Your full name","আপনার পুরো নাম")} className={input} autoFocus autoComplete="name" />
            </div>
            ...
            <button type="submit" disabled={busy} className="w-full rounded-xl py-3.5 font-bold text-white bg-gradient-to-b from-brand to-brand-dark shadow-[0_10px_24px_-8px_rgba(47,144,204,0.55)] active:scale-[0.99] transition disabled:opacity-60 flex items-center justify-center gap-2">
              {busy ? <Spinner /> : L("Done","সম্পন্ন করুন")}
            </button>
          </form>
        ) : (
        <>
        ...
        {tab === "password" ? (
          <form onSubmit={passwordLogin} className="space-y-3">
            <div className="relative">
              ...
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder={L("Enter your email","আপনার ইমেইল দিন")} className={input} autoComplete="email" />
            </div>
            <div className="relative">
              ...
              <input type={show ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder={L("Enter your password","আপনার পাসওয়ার্ড দিন")} className={input + " pr-11"} autoComplete="current-password" />
              <button type="button" onClick={() => setShow((s) => !s)} aria-label={show ? L("Hide","লুকান") : L("Show","দেখুন")} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
                {show
            ...
            <div className="text-right">
              <a href="/account/forgot" className="text-[13px] font-medium text-gray-500 hover:text-brand">{L("Forgot password?","পাসওয়ার্ড ভুলে গেছেন?")}</a>
            </div>
            {err && <p className="rounded-lg bg-red-50 text-red-600 text-sm px-3 py-2">{err}</p>}
            <button type="submit" disabled={busy} className="w-full rounded-xl py-3.5 font-bold text-white bg-gradient-to-b from-brand to-brand-dark shadow-[0_10px_24px_-8px_rgba(47,144,204,0.55)] hover:shadow-[0_14px_30px_-8px_rgba(47,144,204,0.7)] active:scale-[0.99] transition disabled:opacity-60 flex items-center justify-center gap-2">
              {busy ? <Spinner /> : L("Log In","লগইন করুন")}
            </button>
          </form>
        ) : (
          <form onSubmit={otpSent ? verifyOtp : sendOtp} className="space-y-3">
            <div className="relative">
              ...
              <input value={phone} onChange={(e) => setPhone(e.target.value)} inputMode="numeric" placeholder={L("Mobile number (01XXXXXXXXX)","মোবাইল নম্বর (01XXXXXXXXX)")} className={input} autoComplete="tel" disabled={otpSent} />
            </div>
            {otpSent && (
              <div className="relative">
                ...
                <input value={code} onChange={(e) => setCode(e.target.value)} inputMode="numeric" placeholder={L("Enter OTP code","OTP কোড দিন")} className={input + " tracking-[0.3em] font-semibold"} autoComplete="one-time-code" autoFocus />
              </div>
            )}
            {err && <p className="rounded-lg bg-red-50 text-red-600 text-sm px-3 py-2">{err}</p>}
            {msg && <p className="rounded-lg bg-green-50 text-green-700 text-sm px-3 py-2">{msg}</p>}
            <button type="submit" disabled={busy} className="w-full rounded-xl py-3.5 font-bold text-white bg-gradient-to-b from-brand to-brand-dark shadow-[0_10px_24px_-8px_rgba(47,144,204,0.55)] hover:shadow-[0_14px_30px_-8px_rgba(47,144,204,0.7)] active:scale-[0.99] transition disabled:opacity-60 flex items-center justify-center gap-2">
              {busy ? <Spinner /> : otpSent ? L("Verify","যাচাই করুন") : L("Send Code","কোড পাঠান")}
            </button>
            {otpSent && <button type="button" onClick={() => { setOtpSent(false); setCode(""); setMsg(null); }} className="w-full text-center text-[13px] font-medium text-gray-500 hover:text-brand">{L("← Change number","← নম্বর পরিবর্তন করুন")}</button>}
          </form>
        )}

        <p className="mt-4 text-center text-sm text-gray-500">
          {L("No account?","অ্যাকাউন্ট নেই?")} <a href="/account/register" className="font-bold text-brand-dark hover:underline">{L("Sign up","সাইন আপ")}</a>
        </p>

        <div className="mt-5 flex items-center gap-3 text-gray-400 text-[13px]">
          <span className="h-px flex-1 bg-black/10" /> {L("or","অথবা")} <span className="h-px flex-1 bg-black/10" />
        </div>
        ...
        <button onClick={oauthGoogle} disabled={busy}
          ...
          {L("Continue with Google","Google দিয়ে লগইন")}
        </button>
        ...
        <p className="mt-4 flex items-center justify-center gap-1.5 text-[11px] text-gray-400">
          ...
          {L("Your information is fully secure","আপনার তথ্য সম্পূর্ণ সুরক্ষিত")}
        </p>
        </>
        )}
      </div>
