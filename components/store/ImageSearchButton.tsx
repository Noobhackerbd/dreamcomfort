...
export function ImageSearchButton() {
  ...
  return (
    <>
      <button onClick={() => inputRef.current?.click()} aria-label={L("Search by image", "ছবি দিয়ে খুঁজুন")} title={L("Search by image", "ছবি দিয়ে খুঁজুন")}
      ...
      {toast && (
        <div className="fixed left-1/2 -translate-x-1/2 top-16 z-[95] rounded-full bg-gray-900/90 text-white text-[13px] px-4 py-2 shadow-lg">
          {L("Image search coming soon", "ছবি দিয়ে খোঁজা শীঘ্রই আসছে")}
        </div>
      )}
    </>
  );
